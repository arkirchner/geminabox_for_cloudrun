...
# Use Rack::Protection to prevent XSS and CSRF vulnerability if your geminabox server is open
# public.
# # Rack::Protection requires a session middleware, choose your favorite one such as
...
use Rack::Auth::Basic, "GemInAbox" do |username, password|
  ENV.fetch("USERNAME") == username && ENV.fetch("PASSWORD") == password
end
